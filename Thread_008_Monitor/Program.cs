public class Program
{
	...
	public static class Konto
	{
		...
		public static void Auszahlen(int betrag)
		{
			try
			{
				//Lock hat Monitor implementiert, ansonsten genau das gleiche
				//Monitor hat mehr Funktionen als Lock
				Monitor.Enter(LockFlagAuszahlen);

				Kontostand -= betrag;
				TransactionCount++;
				Console.WriteLine($"Kontostand: {Kontostand}");

				Monitor.Exit(LockFlagAuszahlen);
			}
			catch (Exception)
			{
				Console.WriteLine("Deadlock");
		...
		public static void Auszinsen(int prozent)
		{
			try
			{
				//TryEnter gibt bool zurück ob lock funktioniert hat
				//TryEnter = true -> wird gelockt
				if (Monitor.TryEnter(LockFlagAuszahlen))
				{
					Kontostand *= prozent;
					TransactionCount++;
					Console.WriteLine($"Kontostand: {Kontostand}");
					...
					Console.WriteLine("Kann nicht locken");
			}
			catch (Exception)
			{
				Console.WriteLine("Deadlock");
